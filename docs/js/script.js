...
(function moduleExporter(name, closure) {
    ...
    options) {
        // @arg Object = {} - { WEB_VIEW, DISPLAY_DPR, DISPLAY_LONG, DISPLAY_SHORT }
    ...
    options) {
        ...
        var touch3D = /^(iPhone 6s|iPhone 7)/.test(device) || parseFloat(osVersion) >= 10 && /^iPad Pro/.test(device);
    ...
    function _detectBrowserVersion(ua, browser) {
        switch (browser) {
            ...
            case "IE":
                return (/IEMobile/.test(ua) ? _getVersion(ua, "IEMobile/") : /MSIE/.test(ua) ? _getVersion(ua, "MSIE ") // IE 10
                    : _getVersion(ua, "rv:")
                ); // IE 11
            case "Safari":
                return _getVersion(ua, "Version/");
    ...
    function _getiOSDevice(ua, retina, longEdge, osVersion) {
        ...
        if (/iPhone/.test(ua)) {
            ...
            return !retina ? "iPhone 3GS" : longEdge <= 480 ? SGX543 || osVersion >= 8 ? "iPhone 4s" : "iPhone 4" : // iPhone 4 stopped in iOS 7.
            ...
            : SGX543 ? "iPad 3" : SGX554 ? "iPad 4" : A7 ? "iPad mini 2" // iPad mini 3, iPad Air
            : A8X ? "iPad Air 2" : A8 ? "iPad mini 4" : A9X ? longEdge <= 1024 ? "iPad Pro 9.7" : "iPad Pro" : A9 ? "iPad 5" : "iPad x"; // Unknown device (maybe new iPad)
    ...
    function _isWebView(ua, os, browser, version, options) {
        ...
        switch (os + browser) {
            ...
            case "AndroidChrome":
                return parseFloat(version) >= 42 ? /; wv/.test(ua) : /\d{2}\.0\.0/.test(version) ? true // 40.0.0, 37.0.0, 36.0.0, 33.0.0, 30.0.0
    ...
    function _detectFeaturePhoneCarrier(ua) {
        switch (true) {
            ...
            case /KDDI/.test(ua):
                return "KDDI";
            case /SoftBank|Vodafone/.test(ua):
                return "SOFTBANK";
        }
        return "";
    }
...
var ua = new _UserAgent2.default();
var target = document.getElementsByClassName('hoge')[0];
console.log(ua.WEB_VIEW);
target.innerHTML = 'WebView\u3067\u3059\u304B\uFF1F -> ' + ua.WEB_VIEW;

},{"./lib/Sample":3,"./lib/UserAgent":4,"jquery":2}]},{},[5]);
